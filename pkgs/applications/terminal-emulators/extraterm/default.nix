...
#    #ls -la deps/electron-rebuild
#    #ls -la node_modules/electron-rebuild
#    yarn run --offline electron-rebuild
...
stdenv.mkDerivation rec {
  ...
  installPhase = ''
    ...
    patchelf --set-interpreter "$(cat $NIX_CC/nix-support/dynamic-linker)" "$out/opt/extraterm/extraterm"
    patchelf --set-interpreter "$(cat $NIX_CC/nix-support/dynamic-linker)" --set-rpath "${libPath}:$out/opt/extraterm:\$ORIGIN" "$out/opt/extraterm/extraterm_main"
    patchelf --set-interpreter "$(cat $NIX_CC/nix-support/dynamic-linker)" --set-rpath "${libPath}:$out/opt/extraterm:\$ORIGIN" "$out/opt/extraterm/resources/app/extraterm/resources/list-fonts-json-binary/linux-x64/list-fonts-json"
    mv usr/* "$out/"
