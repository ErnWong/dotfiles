...
rec {
  ...
  mkYarnModules =
    ...
    let
      extraBuildInputs = (lib.flatten (builtins.map
        (key:
  ...
  mkYarnPackage =
    ...
    in
    stdenv.mkDerivation (builtins.removeAttrs attrs [ "yarnNix" "pkgConfig" "workspaceDependencies" ] // {
      inherit src pname;
      ...
      configurePhase = attrs.configurePhase or ''
        ...
        # HACK (@ErnWong) - remove extraneous link that was conflicting with existing in extraterm
        # See github.com/nix-community/yarn2nix/pull/106
