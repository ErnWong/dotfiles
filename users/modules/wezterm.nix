# From: https://github.com/nix-community/home-manager/pull/1997
# By L3afMe with rycee's patch partially applied

{ config, lib, pkgs, ... }:
with lib;
let
  cfg = config.programs.wezterm-personal;
  ...
  toWeztermBindings = bindings: isKeyBinding:
    let
      ...
      mapBinding = binding: ''
        ...
        ${toBindingAction binding}},'';
    in
    "${name}={${toString (map mapBinding bindings)}},";
          ...
          let
            default =
              if tabType == "active" then
                ...
                "#3B3052";
          in
          mkColor default "The color of the background area for the tab.";
          ...
          let
            default =
              if tabType == "active" then
                ...
                "#909090";
          in
          mkColor default "The color of the text for the tab.";
  ...
  mkBase16Color = with types;
    colorName: isAnsi: defaultAnsi: defaultBright:
      ...
      in
      mkColor default "The ${name} ${colorName} color.";
...
in
{
  ...
  options.programs.wezterm-personal = with types; {
